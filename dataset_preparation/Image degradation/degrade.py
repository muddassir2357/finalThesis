...
def get_blur(img, degrade_dict):
    # print("blurring image... image size: ",img.shape)
...
def get_down(img, degrade_dict):
    # print("downsampling image... image size: ",img.shape)
    sf = degrade_dict["sf"]
...
def get_noise(img, degrade_dict):
    # print("adding noise... image size: ",img.shape)
...
def get_jpeg(img, degrade_dict):
    # print("adding jpeg compression noise... image size: ",img.shape)
    qf = degrade_dict["qf"]
...
def get_camera(img, degrade_dict):
    h,w,c = img.shape
    img = cv2.resize(img, ((h//2)*2, (w//2)*2))
    # print("adding camera noise... image size: ",img.shape)
...
def get_restore(img, h, w, degrade_dict):
    # print("restoring image... image size: ","image hxw: ",h,"x",w)
    need_shift = degrade_dict["need_shift"]
    sf = degrade_dict["sf"]
    # img1 = cv2.resize(img, (int(h/2), int(w/2)), interpolation=cv2.INTER_CUBIC)
    # img2 = cv2.resize(img, (int(h/3), int(w/3)), interpolation=cv2.INTER_CUBIC)
    img3 = cv2.resize(img, (int(h/4), int(w/4)), interpolation=cv2.INTER_CUBIC)
    if need_shift:
        # img1 = shift_pixel(img1, int(sf))
        # img2 = shift_pixel(img2, int(sf))
        img3 = shift_pixel(img3, int(sf))
...
if __name__ == "__main__":
    ...
    folder="../../dataset/yolo_training/val"
    ...
    for file in files:
        ...
        if img is not None:
            count=count+1
            print("working on ",count, " of ", totalFiles)
            img3 = degradation_pipeline(img)
            # cv2.imwrite("./degradedImages/lqx2/"+file.split(".jpg")[0]+"_lqx2.jpg", img1)
            # cv2.imwrite("./degradedImages/lqx3"+file.split(".jpg")[0]+"_lqx3.jpg", restore_img)
            cv2.imwrite("./degradedImages_val/"+file.split(".jpg")[0]+"_lqx4.jpg", img3)
            print("image saved with name: ",file,"and size: ",img3.shape )